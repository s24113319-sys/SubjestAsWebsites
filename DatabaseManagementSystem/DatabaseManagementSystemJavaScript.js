<!-- This is JavaScript -->
// listing let here so they're in the global scope
let cards, nCards, cover, openContent, openContentText, pageIsOpen = false,
...
function attachListenerToCard(i) {
  cards[i].addEventListener('click', function(e) {
    let card = getCardElement(e.target);
    onCardClick(card, i);
  })
}
...
function scaleCoverToFillWindow(cardPosition) {
  ...
  let scaleX = windowWidth / cardPosition.width;
  let scaleY = windowHeight / cardPosition.height;
  let offsetX = (windowWidth / 2 - cardPosition.width / 2 - cardPosition.left) / scaleX;
  let offsetY = (windowHeight / 2 - cardPosition.height / 2 - cardPosition.top) / scaleY;
...
function resize() {
  if (pageIsOpen) {
    ...
    let cardPosition = currentCard.getBoundingClientRect();
    setCoverPosition(cardPosition);
    scaleCoverToFillWindow(cardPosition);
  }
